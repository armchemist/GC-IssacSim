#!/usr/bin/env bash
# Keyboard teleoperation of OMX_F on rail in Isaac Sim 5.1 with WebRTC streaming.
# Requires model/omx_f_rail.usd — run run_load_omx_f_rail.sh first if absent.
set -e
...
exec python "$PROJECT_DIR/teleop_omx_rail.py" \
