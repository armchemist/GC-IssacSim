...
`add_lab_environment()` → `add_lab_props()` 순서로 런타임에 씬 구성 (레일 배치는
URDF에 bake, 나머지 소품은 매 실행 생성).

**벤치:** Simple_Room 기본 둥근 테이블(`table_low`)은 자동 비활성화되고, 사각
4-다리 벤치(`/World/LabBench`, 1.3×0.9 m, 상판 높이 0.30 m)가 원점에 생성됨.
긴 변 = X. 크기·높이는 `lab_scene.BENCH_SIZE` / `DEFAULT_BENCH` 상수로 조정.

**로봇 배치 (URDF에 bake):** 레일이 벤치 상판(z=0.30) 위, 긴 변(X)을 따라 **끝에서
끝까지(1.25 m)** 놓이고 두 팔은 벤치 건너편(−Y)을 바라봄. 이 배치는
`model/omx_f_dual_on_rail.urdf`의 `world_to_rail_base` 조인트 origin + 팔 mount
−90° 회전으로 **URDF에 직접 baked** 됨 (런타임 transform 아님 — `fix_base`로 물리
베이스가 확실히 고정). 레일 이동범위 0–1.10 m (`rail_joint` upper, teleop `RAIL_MAX`).
벤치 위치·높이를 바꾸면 URDF의 `world_to_rail_base` origin과 `DEFAULT_BENCH`를 함께 수정.

**화학 스테이션 소품 (`add_lab_props`, 벤치 X축 존별 배치):**
- **좌측 시약존** — 색색 시약병 10개(2단 선반, rigid body, `add_reagent_shelf`)
  + 시험관대 1개(정적 프레임) + 시험관 5개(rigid, 슬롯에 세워짐).
- **중앙 믹싱존** — 비커 3개(반투명 rigid body).
- **우측 저울** — 디지털 저울 프롭(`add_scale`, 베이스+디스플레이+원형 플랫폼) 위에
  비커 1개 얹힘.
- **삼각대 카메라** — 벤치 앞-좌 바닥에 삼각대(다리3+바디+렌즈) + 실제
  `UsdGeomCamera`가 믹싱존을 응시 (`add_tripod_camera`). 뷰포트로 사용 가능.

시약병·시험관·비커는 그리퍼로 집기/넘어뜨리기 가능. 안 집히면 `lab_scene.py`의
`mass`/`radius`/collision 튜닝. 존 위치는 `add_lab_props` 안의 X 오프셋으로 조정.
